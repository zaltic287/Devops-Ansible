...
Attention script à lancer en sudo avec docker
avec docker d'installé sur la machine
...
createNodes() {
	...
	docker run -tid --privileged --publish-all=true -v /srv/data:/srv/html -v /sys/fs/cgroup:/sys/fs/cgroup:ro --name $USER-debian-$i -h $USER-debian-$i docker.io/priximmo/buster-systemd-ssh
		docker exec -ti $USER-debian-$i /bin/sh -c "sudo useradd -m -p sa3tHJ3uYvI $USER"
		docker exec -ti $USER-debian-$i /bin/sh -c "sudo mkdir -p ${HOME}/.ssh && sudo chmod 700 ${HOME}/.ssh && sudo chown $USER:$USER $HOME/.ssh"
